...
const filterTabs = ["All", "Pending", "Approved", "Completed", "Rejected"]
...
export default function FarmerOrders() {
  ...
  const fetchOrders = async (farmerId) => {
    try {
      setLoading(true)
      const response = await axios.get(`http://localhost:5000/api/community-medicine-orders/farmer/${farmerId}`)
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Please login as a farmer to view your orders.</p>
        </div>
      </div>
    )
  }
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              ...
              <p className="text-gray-600">
                Track and manage your community medicine requests
              </p>
            </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">{order.quantityRequested} units</span>
                            </div>
                            {order.medicineId?.manufacturer && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Manufacturer:</span>
                                <span className="font-medium">{order.medicineId.manufacturer}</span>
                              </div>
                            )}
                            {order.medicineId?.composition && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Composition:</span>
                                <span className="font-medium">{order.medicineId.composition}</span>
                              </div>
                            )}
                            {order.medicineId?.expiryDate && (
                              <div className="flex justify-between">
                                ...
                                <span className="font-medium">
                                  {new Date(order.medicineId.expiryDate).toLocaleDateString()}
                                </span>
                              </div>
                            )}
                          </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">{order.storeId?.email || "Medical Store"}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-gray-600">Organization:</span>
                              <span className="font-medium">{order.organizationName || "Community Bank"}</span>
                            </div>
                            {order.farmerLocation && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Your Location:</span>
                                <span className="font-medium">{order.farmerLocation}</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="space-y-3">
                          ...
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              ...
                              <span className="font-medium">{formatDate(order.requestDate)}</span>
                            </div>
                            {order.responseDate && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Response Date:</span>
                                <span className="font-medium">{formatDate(order.responseDate)}</span>
                              </div>
                            )}
                            {order.completionDate && (
                              <div className="flex justify-between">
                                <span className="text-gray-600">Completed Date:</span>
                                <span className="font-medium">{formatDate(order.completionDate)}</span>
                              </div>
                            )}
                          </div>
                        </div>
                      ...
                      {(order.farmerNotes || order.storeNotes) && (
                        <div className="border-t pt-4">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {order.storeNotes && (
                              <div>
                                <h5 className="font-medium text-gray-900 mb-2">Store Response:</h5>
